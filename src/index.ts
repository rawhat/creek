...
type HasListener<S, Event extends string> =
  | (S extends {
      addListener: (event: Event, listener: (...args: any[]) => any) => any;
    }
      ? S
      : never)
  | (S extends {
...
type ListenerArguments<S, Event> =
  | (S extends {
      addListener: (event: Event, listener: (...args: infer F) => any) => any;
    }
      ? F
      : never)
  | (S extends {
      addEventListener: (
        event: Event,
        listener: (...args: infer F) => any
      ) => any;
    }
      ? F
      : never);

export function fromEvent<Event extends string, S>(
  target: HasListener<S, Event>,
  type: Event
) {
